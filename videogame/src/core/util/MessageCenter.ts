...
class MessageCenter extends BaseClass {
    ...
    public register(type:number,cmd:number,listener:Function, listenerObj:any){
        this.addListener(type + "_" + cmd,listener,listenerObj);
    }
    public removeRegister(type:number,cmd:number,listener:Function, listenerObj:any){
        this.removeListener(type + "_" + cmd,listener,listenerObj);
    }
    /**
     * 添加消息监听
     * @param type 消息唯一标识
